...
def unicodify(s, encoding='utf8', *args):
  return s
  # NOTE(dounanshi): this code results in an unicode error.
...
def tokenize(tweet):
  ...
  # NOTE(dounanshi): filter out hashtags, twitter names, and urls
  t = [token.lower() for token in t if not token.startswith(("@", "#", "http"))]
  return t
